...
window.onload = function () {
	...
	// Hiện thông báo "chờ xử lý đơn hàng" khi vừa chuyển từ trang thanh toán (sau khi đặt hàng / gửi bill)
	var params = new URLSearchParams(window.location.search);
	if (params.get('pending') === '1') {
		addAlertBox('Đơn hàng của bạn đang chờ xử lý. Quân Lan Mobile sẽ liên hệ xác nhận với bạn sớm.', '#1e88e5', '#fff', 6000);
		if (window.history && window.history.replaceState) {
			window.history.replaceState({}, '', 'giohang.html');
		}
	}
}
...
function addProductToTable(user) {
    ...
    // START HIGH PRIORITY IMPLEMENT - STICKY CHECKOUT LOGIC
    setupStickyCheckout(totalPrice);
    // END HIGH PRIORITY IMPLEMENT - STICKY CHECKOUT LOGIC
}

/* START HIGH PRIORITY IMPLEMENT - STICKY CHECKOUT LOGIC */
function setupStickyCheckout(totalPrice) {
    ...
    if (stickyBar && stickyTotal) {
        ...
        stickyTotal.textContent = numToString(totalPrice) + '₫';

        const originalThanhToan = document.querySelector('.thanhtoan');
        
        window.addEventListener('scroll', () => {
            if (!originalThanhToan) return;
            const rect = originalThanhToan.getBoundingClientRect();
            // Show sticky bar when the original checkout button is off-screen (scrolled past)
            if (rect.bottom < 0) {
