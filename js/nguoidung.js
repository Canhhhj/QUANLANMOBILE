...
window.onload = function () {
    ...
    if (currentUser) {
        ...
        var warning = `<h2 class="nd-warning">Bạn chưa đăng nhập !!</h2>`;
...
function addTatCaDonHang(user) {
    if (!user) {
        document.getElementsByClassName('listDonHang')[0].innerHTML = `
            <h3 class="nd-warning">Bạn chưa đăng nhập !!</h3>`;
        return;
    }
    if (!user.donhang.length) {
        document.getElementsByClassName('listDonHang')[0].innerHTML = `
            <h3 class="nd-success">Xin chào ` + currentUser.username + `. Bạn chưa có đơn hàng nào.</h3>`;
        return;
    }
