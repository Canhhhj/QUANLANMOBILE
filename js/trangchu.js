window.onload = function () {
	...
	for (var c of company) addCompany("assets/img/company/" + c, c.slice(0, c.length - 4));
	...
	if (filters.length) { // có filter
		...
		var soLuong = (window.innerWidth < 1200 ? 4 : 5); // 1 hàng thôi: 4 hoặc 5 sp cho gọn
	...
	for (var i = 0; i < sortTabs.length; i++) {
		...
		if (currentSort && href.indexOf('sort=' + currentSort) >= 0) tab.classList.add('active');
		else if (!currentSort && (!href || href === '')) tab.classList.add('active');
	}
...
function addKhungSanPham(tenKhung, color, filter, len, ele) {
	...
	s += `	</div>
			<a class="xemTatCa" href="index.html?` + filter.join('&') + `">Xem tất cả ` + spResult.length + ` sản phẩm</a>
		</div>`;
		...
		if (groups[groupKey]) {
			...
			switch (groupKey) {
				case 'gia_re':
					...
					break;
				case 'giam_gia_lon':
					match = sp.promo && sp.promo.name === 'giamgia';
					break;
				case 'gia_soc_online':
					match = sp.promo && sp.promo.name === 'giareonline';
					break;
				case 'tra_gop_0':
					match = sp.promo && sp.promo.name === 'tragop';
					break;
				case 'san_pham_moi':
					match = sp.promo && sp.promo.name === 'moiramat';
					break;
				case 'noi_bat_nhat':
					match = sp.star >= 3;
					break;
...
function addCompany(img, nameCompany) {
	...
	var new_tag = `<a href=` + link + `><img src=` + fixImagePath(img) + `></a>`;
...
function toggleFilterPanel() {
	...
	document.body.classList.toggle('filter-panel-open', panel.classList.contains('open'));
}
