...
window.onload = function () {
    ...
    // Breadcrumb
    if (typeof renderBreadcrumb === 'function' && nameProduct) {
        renderBreadcrumb([
            { text: 'Trang chủ', url: 'index.html' },
            { text: 'Điện thoại', url: 'index.html' },
            { text: nameProduct, url: null }
        ]);
    }
    ...
    // START HIGH PRIORITY IMPLEMENT - STICKY CTA LOGIC
    setupStickyCTA();
    // END HIGH PRIORITY IMPLEMENT - STICKY CTA LOGIC
}
...
function setupStickyCTA() {
    ...
    if (stickyBar && stickyImg && stickyName && stickyPrice) {
        stickyImg.src = fixImagePath(sanPhamHienTai.img);
        stickyName.textContent = sanPhamHienTai.name;
        stickyPrice.textContent = (sanPhamHienTai.promo.name === 'giareonline' ? sanPhamHienTai.promo.value : sanPhamHienTai.price) + '₫';

        const originalCTA = document.querySelector('.area_order');
        
        window.addEventListener('scroll', () => {
            if (!originalCTA) return;
            const rect = originalCTA.getBoundingClientRect();
            // Show sticky bar when the original buy button is off-screen (scrolled past)
            if (rect.bottom < 0) {
...
function phanTich_URL_chiTietSanPham() {
    ...
    // Đổi title
    document.title = nameProduct + ' - QUÂN LAN MOBILE';
    ...
    hinh.src = fixImagePath(sanPhamHienTai.img);
    document.getElementById('bigimg').src = fixImagePath(sanPhamHienTai.img);

    // Hình nhỏ
    addSmallImg("assets/img/products/huawei-mate-20-pro-green-600x600.jpg");
    addSmallImg("assets/img/chitietsanpham/oppo-f9-mau-do-1-org.jpg");
    addSmallImg("assets/img/chitietsanpham/oppo-f9-mau-do-2-org.jpg");
    addSmallImg("assets/img/chitietsanpham/oppo-f9-mau-do-3-org.jpg");
    addSmallImg("assets/img/products/huawei-mate-20-pro-green-600x600.jpg");
    addSmallImg("assets/img/chitietsanpham/oppo-f9-mau-do-3-org.jpg");
    addSmallImg("assets/img/products/huawei-mate-20-pro-green-600x600.jpg");
...
function addSmallImg(img) {
    var newDiv = `<div class='item'>
                        <a>
                            <img src=` + fixImagePath(img) + ` onclick="changepic(this.src)">
                        </a>
                    </div>`;
